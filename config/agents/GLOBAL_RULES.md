...
- M1. Any agent update to config owned by portable dotfiles repo → invoke `sync-config-aron` before edits, then after completed batch. Scope from owner repo `docs/deployment.md`: editor, agents/skills, shell/tools, desktop/apps, services, scripts/manifests/setup docs. Resolve skill through shared skill discovery; no fixed checkout path.
- M2. Initial bootstrap exception: explicitly authorized creation of dotfiles repo may proceed before remote identity exists; no commits/pushes unless authorized. Report local export, remote unset, synchronization pending; never claim synced. After bootstrap, missing remote blocks publication, not preservation of already-authorized edits. Repo `AGENTS.md` must identify deployment map, validation checks, authoritative bootstrap remote/branch. Missing ownership → report blocker; never assume arbitrary repo owns runtime config. Manual/GUI edits require explicit reconciliation; no watcher implied.
