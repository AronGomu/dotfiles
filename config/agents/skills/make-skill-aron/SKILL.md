...
Resolve owner repo via deployment map; invoke `sync-config-aron` before edits, after batch. Read owner `AGENTS.md`. No fixed checkout path.
...
| name             | value                                                              |
| ---------------- | ------------------------------------------------------------------ |
| Skills root      | `{owner repo}/config/agents/skills/`                        |
...
| Install rule     | `{owner repo}/docs/deployment.md` — shared hub, whole dir |
| Live path        | `~/.agents/skills/{name}/` -> symlinked to claude, pi, codex       |
| Deploy           | Follow owner `AGENTS.md`; no system rebuild                      |
...
- P9. **Install** — follow owner `AGENTS.md` deployment contract. Canonical hub `~/.agents/skills`; source `{owner repo}/config/agents/skills`. Link/copy only approved new dir; preserve existing dirs. Existing hub link exposes new skills without system apply. Verify each harness sees one copy.
- P10. **Report** — files, shape, validation, actual deployment state, invocation (`/{name}`). Invoke sync skill after batch; bootstrap exception may leave commit/remote pending.

**Never run system apply** (K1). Never claim skill installed without discovery check.
...
- R9. No framework wiring per skill. Shared hub is canonical; update deployment map only when discovery behavior changes.
...
- D6. User told deployment state + `/{name}` invocation
- D7. **Not** claimed installed until harness discovery verified
...
- C5. System apply stays user-executed; reversible authorized config sync follows G3/M1–M4.
