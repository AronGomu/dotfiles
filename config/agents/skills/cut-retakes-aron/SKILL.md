...
- R10. GPU runtime + certs come from native target/project venv. No old loader overrides. Use project entrypoints; verify supported CUDA/torch build before GPU render.
- Venv missing/broken -> rebuild per `{tool home}/README.md`. ~7 GB download, run it in background, never inline in a turn.
- R12. K1: system package/driver apply stays user-executed. Project sources remain separate; see owner tools manifest for restore requirements.
