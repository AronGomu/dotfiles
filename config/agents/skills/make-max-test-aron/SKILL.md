...
**Self-contained.** Reads nothing outside `~/.agents/skills/make-max-test-aron/`.

**G11 proof limitation:** inherited `gates/prove-test.sh` remains unverified as
an automated proof gate. Exit 126/127 now fails closed (`CANNOT RUN`, exit 2),
but other infrastructure/import failures can still be mistaken for expected
assertion failures. Agents must NOT rely on G11 PASS as automated proof until
separately repaired and validated. Record independently observed red/green
assertion evidence; do not treat source preservation as gate certification.

## Pre-flight

Read now, both:

- `~/.agents/skills/make-max-test-aron/GATES-FORMAT.md` — gate contract, thresholds, config schema, budgets.
- `~/.agents/skills/make-max-test-aron/ADVISORS-FORMAT.md` — advisor dispatch matrix, fix policy.
...
| name           | value                                                                                  |
| -------------- | -------------------------------------------------------------------------------------- |
| Skill dir      | `~/.agents/skills/make-max-test-aron/`                                                       |
...
```
Read ~/.agents/skills/make-max-test-aron/roles/{role}.md. Follow it fully.
```
