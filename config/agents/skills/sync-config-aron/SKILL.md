...
description: >
  ...
  dotfiles repo: editor, agents, skills, shell, tools, desktop, apps, services,
...
- B1. Explicit initial dotfiles creation with remote/branch unset may create mapped sources, manifests, docs, tests. Run pre/post validation. Do not invent remote or block initial export.
- B2. Stage/commit/push only when expressly authorized; initial export was prepared unstaged for review. Inspect current Git state rather than assuming that historical state persists. While remote remains unset, report `local export checked; remote unset; bootstrap sync pending`. Normal clean/HEAD/remote success criteria remain unmet until configured.
- B3. Subsequent tasks use normal workflow. Missing remote prevents publication claims; preserve authorized edits, report blocker. No blanket exception for unrelated runtime edits.
