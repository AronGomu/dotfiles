...
Read `~/.agents/GLOBAL_RULES.md` now and follow it fully. It carries the global implementation rules, id'd `A1`-`M4`; skills cite those ids instead of restating them.
...
# graphify
- G1. **graphify** (`~/.claude/skills/graphify/SKILL.md`) - any input to knowledge graph. Trigger: `/graphify`
When the user types `/graphify`, use the installed graphify skill or instructions before doing anything else.

Mapped config edits require `sync-config-aron` before edits, after batch. Resolve owner `AGENTS.md` + deployment map. Bootstrap exception applies only while explicitly creating unconfigured dotfiles repo.
