...
| D5 | `config/pi/agent/settings.json` | `~/.pi/agent/settings.json`: writable seed/merge; preserve runtime defaultProvider/defaultModel/defaultThinkingLevel/modelThinkingLevels; union enabledModels. Do not copy auth, trust, sessions, onboarding counters. Other prefs reviewed key-by-key. |
| D6 | `config/pi/agent/{configs,extensions,prompts,keybindings.json,claude-bridge.json,i-have-adhd.json}` | Matching `~/.pi/agent/` paths. Source extensions/prompts may link; configs writable seeds. `i-have-adhd.json` belongs directly under agent dir; enables ADHD mode for new sessions with per-session toggles ([details](pi-adhd.md)). `remember-model` auto-saves interactive model/thinking selections into writable global settings; headless runs excluded (see extension README). `env.example` is documentation, never live secrets. Two Herdr integration files are source snapshots; let current Herdr regenerate managed integration only after comparison, preserve custom sidecar. |
...
- S4. User-authorized existing `origin`: `AronGomu/dotfiles` on GitHub; authoritative bootstrap branch `main`. Public source repo. Root AGENTS.md defines fast-forward publication and corrective-commit recovery; no system apply or watcher implied. Recheck hooks/downstream effects before pushes. Full sync requires fetched remote equality plus whole-worktree cleanliness.

## Tailscale launcher

- T1. `bin/tailscale-open` → `~/.local/bin/tailscale-open` via D14; `desktop/launchers/tailscale-open.desktop` → `$XDG_DATA_HOME/applications/` via D18. Terminal entry supports sudo password prompt and visible failures. Review conflicts before linking/copying.
- T2. Native capabilities: Python 3, Tailscale with `tailscaled.service`, sudo, systemd, iproute2, xdg-utils, browser/terminal. NetworkManager `nmcli` and Mullvad CLI integrate their installed managers. No automatic package/service installation, firewall apply, or runtime VPN disconnect during config deployment.
- T3. `bin/tailscale-open`, `tests/test_tailscale_open.py`, and `docs/tailscale-open.md` mirror Nix repo script (`home/aron/scripts/tailscale-open.py`), test, and usage doc byte-for-byte. Keep copies tested together; runtime never depends on the sibling checkout. See [tailscale-open.md](tailscale-open.md) for safety, supported VPN managers, and unknown-blocker behavior.
